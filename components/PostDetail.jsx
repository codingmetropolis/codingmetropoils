...
const PostDetail = ({ post }) => {
  ...
  const getContentFragment = (index, text, obj, type) => {
    ...
    switch (type) {
      ...
      case "table_head":
        return (
          <thead key={index} className="bg-gray-50">
             {
            obj.children.map((typeObj, index) => {
              // console.log(typeObj,index);
            return getContentFragment(index, typeObj.type, typeObj, typeObj.type);
          })}
          </thead>
        );
      case "table":
        return (<>
        {/* {console.log(obj)} */}
          <table  key={index} className="border-b border-gray-200 shadow">
            {
            obj.children.map((typeObj, index) => {
              // console.log(typeObj,index);
            return getContentFragment(index, typeObj.type, typeObj, typeObj.type);
      ...
      case "table_header_cell":
        return (
          <th key={index} className="px-6 py-2 text-xs text-gray-500 text-lg">
            {
            obj.children.map((typeObj, index) => {
